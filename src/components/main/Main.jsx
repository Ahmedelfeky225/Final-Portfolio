  ...
  const handleClick = (buttonCategory)=>{
    if(buttonCategory === "all"){
      ...
      const filterArr= myProjects.filter((item)=>{
        const filterProject = item.category.find((myItem)=>{return myItem === buttonCategory})
    ...
    <main className="flex">
      <section className="flex left-section" id="projects">
        {buttonCategories.map((btnInfo,i)=>{
          ...
          return (
          <button key={i} className={currentActive === i ? "active":null} onClick={()=>{
            setCurrentActive(i)
      ...
      {
         arr.map((item)=>{
          const {projectTitle,imgPath,projectDesc,linkLiveProject,linkProjectInGithub}=item
          return(
            <motion.article
            layout
      ...
      {showModelMore && <ModalMore dataProject={dataProject} setShowModalMore={setShowModalMore}/>}
    </main>
  )
}
export default Main;               